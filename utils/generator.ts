export const generateScientificNames = (count: number) => {	
	var vowels = {  '1': ["b", "c", "d", "f", "g", "h", "i", "j", "k", "l", "m", "n", "p", "q", "r", "s", "t", "v", "w", "x", "y", "z"],
		...
		mtx = [[1,1, 2,2, 5,5],
			...
			[3,3, 4,4, 1,1, 4,4, 5,5],
			[4,4, 1,1, 4,4, 3,3, 6,6]],
		fn = function(i: number) { return Math.floor(Math.random() * vowels[i as 1].length); },
		ret = [],
		name,
		comp,
		i, il,
	...
	for (; c<count; c++) {
		...
		for (i=0, il=comp.length/2; i<il; i++) {
			name += vowels[comp[i*2] as 1][fn(comp[i*2+1])];
		}
		ret.push(name);
	}
